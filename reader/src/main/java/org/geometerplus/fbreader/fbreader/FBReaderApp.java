...
public final class FBReaderApp extends ZLApplication {
	...
	public volatile List<Bookmark> bookmarks = new ArrayList<Bookmark>();
	...
	private synchronized void openBookInternal(final Book book, Bookmark bookmark, boolean force) {
		...
		if (plugin instanceof ExternalFormatPlugin) {
			...
			if (bookmark != null) {
				...
				bm = new Bookmark(Collection, book, "", new EmptyTextSnippet(pos), false,false);
			}
	...
	public void gotoBookmark(Bookmark bookmark, boolean exactly) {
	...
	public void addInvisibleBookmark(ZLTextWordCursor cursor) {
		...
		updateInvisibleBookmarksList(new Bookmark(
			Collection, book, textModel.getId(), snippet, false,false
		));
	}
	...
	public void addBookmark(){
		Bookmark bookmark = createBookmark(30,true);
		bookmarks.add(bookmark);
		Collection.saveBookmark(bookmark);
	}

	private void updateBookmarks() {
		for (BookmarkQuery query = new BookmarkQuery(Model.Book, 50); ; query = query.next()) {
	...
	public boolean checkCurrentPageHasBookMark() {
		for (Bookmark bookmark : bookmarks) {
			if (bookmark.IsVisible && !bookmark.Highlight) {
				if (bookmark.getEnd() == null) {
					BookmarkUtil.findEnd(bookmark, BookTextView);
				}
				BookmarkHighlighting highlighting = new BookmarkHighlighting(BookTextView, Collection, bookmark);
				if (highlighting.intersects(BookTextView.myCurrentPage)) {
					return true;
				}
			}
		}
	...
	public void deleteBookmark() {
		Iterator<Bookmark> bookmarksIterator = bookmarks.iterator();
		while (bookmarksIterator.hasNext()) {
			...
			BookmarkHighlighting highlighting = new BookmarkHighlighting(BookTextView, Collection, bookmark);
			if (highlighting.intersects(BookTextView.myCurrentPage) && !bookmark.Highlight) {
				bookmarksIterator.remove();
				Collection.deleteBookmark(bookmark);
			}
		}
	}

	public void deleteHighlight(Bookmark bookmark) {
		if (bookmark != null) {
			Collection.deleteBookmark(bookmark);
			getViewWidget().reset();
			getViewWidget().repaint();
    ...
    public TOCTree getPreTOCElement() {
		TOCTree current = getCurrentTOCElement();
		if (current == null || current.getReference() == null) {
			return null;
		}
		int index = getCurrentTOCElement().getReference().ParagraphIndex;
		TOCTree lastTree = null;
	...
	public TOCTree getNextTOCElement() {
		ZLTextWordCursor cursor = BookTextView.getStartCursor();
		if (Model == null || cursor == null) {
			return null;
		}
	...
	public TOCTree getTOCElement(ZLTextWordCursor cursor) {
		if (Model == null || cursor == null) {
			return null;
		}
