...
public final class FBView extends ZLTextView {
    ...
    public void onFingerSingleTap(int x, int y) {
        if (myReader.getPopupById(SelectionPopup.ID).isShowing()) {
            myReader.runAction(ActionCode.SELECTION_CLEAR);
            myReader.hideActivePopup();
            return;
        }
        if (myReader.getPopupById(HighlightPopup.ID).isShowing()) {
            myReader.hideActivePopup();
            return;
        }
    ...
    private class FooterStyle extends Footer {
        ...
        public void paint(ZLPaintContext context) {
            ...
            final int left = getLeftMargin();
            int right = context.getWidth() - getRightMargin();
            final int height = getHeight();
            setFont(context, height, false);

            int oldTextSize = context.getTextSize();
            context.setTextColor(myViewOptions.getColorProfile().MenuTextOption.getValue());

            //画进度
            ZLTextView.PagePosition pagePosition = pagePosition();
            int currentPosition = pagePosition.Current;
            int totalPage = pagePosition.Total;
            float percent = 100f * currentPosition / totalPage;
            String progress = "%1$.1f";
            context.setTextSize(FBReaderPercentUtils.dp2px(12));
            context.drawString(left, height - FBReaderPercentUtils.dp2px(12), String.format(progress, percent) + "%");

            //画电量
            FooterOptions footerOptions = myViewOptions.getFooterOptions();

            if (footerOptions.ShowBattery.getValue()) {
                int batteryWidth = FBReaderPercentUtils.dp2px(19);
                int batteryHeight = FBReaderPercentUtils.dp2px(9);
                int batteryBottom = FBReaderPercentUtils.dp2px(12);
                right -= batteryWidth;
                RectF rectF = new RectF(right, height - batteryHeight - batteryBottom,
                        right + batteryWidth, height - batteryBottom);
                context.setLineColor(myViewOptions.getColorProfile().MenuTextOption.getValue());
                context.setLineWidth(2);
                context.drawRoundRectangle(rectF, 2, 2);
                float batteryMiddle = (rectF.top + rectF.bottom) / 2;

                context.setFillColor(myViewOptions.getColorProfile().MenuTextOption.getValue());
                RectF middle = new RectF(rectF.right, batteryMiddle - batteryHeight / 5, rectF.right + 4, batteryMiddle + batteryHeight / 5);
                context.fillRoundRectangle(middle, 10, 10);

                int innerWidth = (int) ((myReader.getBatteryLevel() / 100f) * (rectF.right - rectF.left - 10));
                RectF inner = new RectF(rectF.left + 5, rectF.top + 5, 0, rectF.bottom - 5);
                inner.right = inner.left + innerWidth;
                context.fillRoundRectangle(inner, 2, 2);
                right -= FBReaderPercentUtils.dp2px(4);
            }

            //画时间
            if (footerOptions.ShowClock.getValue()) {
                final String time = ZLibrary.Instance().getCurrentTimeString();
                final int timeWidth = context.getStringWidth(time);
                context.drawString(right - timeWidth, height - FBReaderPercentUtils.dp2px(12), time);
            }
    ...
    public static final int SCROLLBAR_SHOW_HIDE = 0;
    public static final int SCROLLBAR_SHOW_SHOW = 1;
    public static final int SCROLLBAR_SHOW_AS_PROGRESS = 1;
    public static final int SCROLLBAR_SHOW_AS_FOOTER = 3;
    public static final int SCROLLBAR_SHOW_AS_FOOTER_OLD_STYLE = 4;
    public static final int SCROLLBAR_SHOW_AS_FOOTER_KAO_LA_STYLE = 5;
