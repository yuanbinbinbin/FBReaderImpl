...
final class SQLiteBooksDatabase extends BooksDatabase {
	...
	protected List<Bookmark> loadBookmarks(BookmarkQuery query) {
		...
		final StringBuilder sql = new StringBuilder("SELECT")
			...
			.append("bm.style_id,").append("bm.highlight")
		...
		while (cursor.moveToNext()) {
			list.add(createBookmark(
				...
				query.Visible,cursor.getLong(18) == 1,
	...
	protected long saveBookmark(Bookmark bookmark) {
		...
		if (bookmarkId == -1) {
			statement = get(
				"INSERT INTO Bookmarks (uid,version_uid,book_id,bookmark_text,original_text,creation_time,modification_time,access_time,model_id,paragraph,word,char,end_paragraph,end_word,end_character,visible,highlight,style_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
			);
		} else {
			statement = get(
				"UPDATE Bookmarks SET uid=?,version_uid=?,book_id=?,bookmark_text=?,original_text=?,creation_time=?,modification_time=?,access_time=?,model_id=?,paragraph=?,word=?,char=?,end_paragraph=?,end_word=?,end_character=?,visible=?,highlight=?,style_id=? WHERE bookmark_id=?"
			);
		}
		...
		synchronized (statement) {
			...
			statement.bindLong(++fieldCount, bookmark.Highlight ? 1 : 0);
	...
	private void updateTables40() {
		myDatabase.execSQL("ALTER TABLE Bookmarks ADD COLUMN highlight INTEGER DEFAULT 0");
	}
