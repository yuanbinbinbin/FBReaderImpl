...
public final class ZLTextParagraphCursor {
	private static final class Processor {
		...
		void fill() {
			...
			boolean isTopText = false;
			for (ZLTextParagraph.EntryIterator it = myParagraph.iterator(); it.next(); ) {
				switch (it.getType()) {
					case ZLTextParagraph.Entry.TEXT:
						processTextEntry(it.getTextData(), it.getTextOffset(), it.getTextLength(), hyperlink, isTopText);
						isTopText = false;
						break;
		...
		private void processTextEntry(final char[] data, final int offset, final int length, ZLTextHyperlink hyperlink,boolean isTopText) {
			if (length != 0) {
				...
				for (int index = 0; index < length; ++index) {
					...
					if (Character.isWhitespace(ch)) {
						if (index > 0 && spaceState == NO_SPACE) {
							addWord(data, offset + wordStart, index - wordStart, myOffset + wordStart, hyperlink,false);
						}
						spaceState = SPACE;
						...
						switch (spaceState) {
							...
							case NO_SPACE:
								if (index > 0 &&
									...
									previousChar != '-' &&
									index != wordStart) {
									addWord(data, offset + wordStart, index - wordStart, myOffset + wordStart, hyperlink,isTopText);
									wordStart = index;
								}
								break;
						}
				...
				switch (spaceState) {
					...
					case NON_BREAKABLE_SPACE:
						...
						break;
					case NO_SPACE:
						addWord(data, offset + wordStart, length - wordStart, myOffset + wordStart, hyperlink,isTopText);
						break;
				}
				myOffset += length;
			}
		...
		private final void addWord(char[] data, int offset, int len, int paragraphOffset, ZLTextHyperlink hyperlink,boolean isTopText) {
			...
			if (isTopText) {
				if (myElements != null && myElements.size() > 0 && myElements.get(myElements.size() - 1) instanceof ZLTextWord) {
					((ZLTextWord) myElements.get(myElements.size() - 1)).topWord = word;
				}
			} else {
				myElements.add(word);
			}
