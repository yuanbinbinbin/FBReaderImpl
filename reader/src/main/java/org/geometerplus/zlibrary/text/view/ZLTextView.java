...
import org.geometerplus.fbreader.fbreader.TTSReadHighlighting;
...
public abstract class ZLTextView extends ZLTextViewBase {
	...
	public void bottomLine(ZLTextPosition start, ZLTextPosition end){
		removeHighlightings(TTSReadHighlighting.class);
		addHighlighting(new TTSReadHighlighting(this, start, end));
	}

	public void clearBottomLine(){
		if (removeHighlightings(TTSReadHighlighting.class)) {
			Application.getViewWidget().reset();
			Application.getViewWidget().repaint();
		}
	}
	public void moveSelectionCursorTo(SelectionCursor.Which which, int x, int y) {
	...
	private ZLTextSelection.Point getSelectionCursorPoint(ZLTextPage page, SelectionCursor.Which which) {
		...
		if (which == SelectionCursor.Which.Left) {
			...
			if (area != null) {
				return new ZLTextSelection.Point(area.XStart, area.YEnd - area.Style.getFontSize(metrics()) - getContext().getDescent() -2);
			}
		} else {
			...
			if (area != null) {
				return new ZLTextSelection.Point(area.XEnd, area.YEnd + 2);
			}
		}
		return null;
	}
	...
	public SelectionCursor.Which findSelectionCursor(int x, int y) {
	...
	public synchronized void paint(ZLPaintContext context, PageIndex pageIndex) {
		...
		for (ZLTextHighlighting h : hilites) {
			...
			final ZLColor bottomLineColor = h.getBottomLineColor();
			if (bottomLineColor != null) {
				context.setLineColor(outlineColor);
				mode |= Hull.DrawMode.BottomLine;
			}
    ...
    private void drawHeader(ZLPaintContext context, ZLTextPage pageIndex) {
        if (context == null || pageIndex == null) {
            return;
        }
        FBReaderApp fbReaderApp = ((FBReaderApp) FBReaderApp.Instance());
        TOCTree currentTree = fbReaderApp.getTOCElement(pageIndex.StartCursor);
        //画章节名
        if (fbReaderApp.ViewOptions.HeaderShowCatalogName.getValue() && currentTree != null && currentTree.getText() != null) {
            int oldTextSize = context.getTextSize();
            context.setTextSize(FBReaderPercentUtils.dp2px(12));
            context.setTextColor(fbReaderApp.ViewOptions.getColorProfile().MenuTextOption.getValue());
            context.drawString(getLeftMargin(), FBReaderPercentUtils.dp2px(20), currentTree.getText());
            context.setTextSize(oldTextSize);
        }
		...
		synchronized (myHighlightings) {
			for (ZLTextHighlighting h : myHighlightings) {
				if (h.intersects(pageIndex) && h instanceof BookmarkHighlighting && h.getBackgroundColor() == null) {
					hasBookMark = true;
					break;
				}
			}
		...
		if (hasBookMark) {
			int width = FBReaderPercentUtils.dp2px(40);
			Bitmap bitmap = BitmapFactory.decodeResource(fbReaderApp.getReader().getApplicationContext().getResources(), R.drawable.ic_fbreader_book_mark_tag);
			context.drawImage(FBReaderPercentUtils.getScreenWidth() - width - getRightMargin(), width,
					BitmapImageData.get(new ZLBitmapImage(bitmap)),
					new ZLPaintContext.Size(width, width), ZLPaintContext.ScalingType.FitMaximum, ZLPaintContext.ColorAdjustingMode.NONE);
		}
		...
		for (int wordIndex = info.RealStartElementIndex; wordIndex != endElementIndex && index < to; ++wordIndex, charIndex = 0) {
			...
			if (element == area.Element) {
				...
				if (element instanceof ZLTextWord) {
					...
					ZLTextWord topWord = ((ZLTextWord) element).topWord;
					if (topWord != null) {
						ZLTextStyle textStyle = getTextStyle();
						applyTopWordStyle();
						ZLTextElementArea topArea = ((ZLTextWord) element).topArea;
						final int topAreaX = topArea.XStart;
						final int topAreaY = topArea.YEnd - getElementDescent(element) - getTextStyle().getVerticalAlign(metrics());
						drawWord(
								topAreaX, topAreaY, topWord, charIndex, -1, false,
								hlColor != null ? hlColor : getTextColor(getTextStyle().Hyperlink)
						);
	...
	private void buildInfos(ZLTextPage page, ZLTextWordCursor start, ZLTextWordCursor result) {
		...
		do {
			...
			while (info.EndElementIndex != endIndex) {
				...
				//textAreaHeight递减，代表屏幕上能显示的总高度在减少
	...
	//计算出屏幕上每一行中的第一个字与最后一个字在这个ArrayList中的位置
	...
	private ZLTextLineInfo processTextLineInternal(
		...
		do {
			ZLTextElement element = paragraphCursor.getElement(currentElementIndex);
			int elementWidth = getElementWidth(element, currentCharIndex);
			if (element instanceof ZLTextWord) {
				ZLTextWord topWord = ((ZLTextWord) element).topWord;
				if (topWord != null) {
					ZLTextStyle textStyle = getTextStyle();
					applyTopWordStyle();
					int topWordWidth = getElementWidth(topWord, 0);
					setTextStyle(textStyle);
					newWidth += (topWordWidth > elementWidth ? topWordWidth : elementWidth);
				} else {
	...
	private void prepareTextLine(ZLTextPage page, ZLTextLineInfo info, int x, int y, int columnIndex) {
		...
		for (int wordIndex = info.RealStartElementIndex; wordIndex != endElementIndex; ++wordIndex, charIndex = 0) {
			...
			if (element == ZLTextElement.HSpace) {
				...
				int topTextWidth = 0;
				int topTextHeight ;
				ZLTextWord topWord = null;
				if (element instanceof ZLTextWord) {
					topWord = ((ZLTextWord) element).topWord;
					if (topWord != null) {
						ZLTextStyle textStyle = getTextStyle();
						applyTopWordStyle();
						topTextWidth = getElementWidth(topWord, 0);
						topTextHeight = getElementHeight(topWord);
						setTextStyle(textStyle);
						int topWordX = x;
						if (topTextWidth < width) {
							topWordX = x + (width - topTextWidth) / 2;
						}
						((ZLTextWord) element).topArea = new ZLTextElementArea(
								paragraphIndex, wordIndex, 0,
								topWord.Length,
								true, // is last in element
								false, // add hyphenation sign
								changeStyle, getTextStyle(), element,
								topWordX, topWordX + topTextWidth - 1, y - height-height, y + descent, columnIndex
						);
					}
				}
				int startX = x;
				if (width < topTextWidth) {
					startX = x + (topTextWidth - width) / 2;
					width = topTextWidth;
				}
				page.TextElementMap.add(new ZLTextElementArea(
					...
					false, // add hyphenation sign
					changeStyle, getTextStyle(), element,
					startX, startX + width - 1, y - height + 1, y + descent, columnIndex
				));
				changeStyle = false;
				wordOccurred = true;
			} else if (isStyleChangeElement(element)) {
...
*/
	...
	public boolean initSelection(int x, int y) {
		y -= getTextStyleCollection().getBaseStyle().getFontSize() / 2;
	 ...
	 * @return
	 */
    public List<TTSReadAction.TTSLineInfo> getCurrentPageText() {
        return getPageText(myCurrentPage);
    }
	 ...
	 * @return
	 */
	public String getCurrentPageTextString() {
		StringBuilder result = new StringBuilder();
		try {
			if (myCurrentPage != null && !myCurrentPage.isEmptyPage()) {
				ArrayList<ZLTextLineInfo> lines = myCurrentPage.LineInfos;
				if (lines != null) {
					for (ZLTextLineInfo line : lines) {
						if (line != null) {
							ZLTextParagraphCursor paragraph = line.ParagraphCursor;
							int startElement = line.StartElementIndex;
							int endElement = line.EndElementIndex;
							for (int i = startElement; i < endElement; i++) {
								ZLTextElement element = paragraph.getElement(i);
								if (element instanceof ZLTextWord) {
									ZLTextWord word = (ZLTextWord) element;
									result.append(new String(word.Data, word.Offset, word.Length));
								}
							}
						}
					}
     ...
     * @return
     */
    public List<TTSReadAction.TTSLineInfo> getNextPageText() {
		if (myNextPage.PaintState == PaintStateEnum.NOTHING_TO_PAINT) {
			myNextPage.StartCursor.setCursor(getEndCursor());
			myNextPage.PaintState = PaintStateEnum.START_IS_KNOWN;
			preparePaintInfo(myNextPage);
		}
        return getPageText(myNextPage);
    }

	private List<TTSReadAction.TTSLineInfo> getPageText(ZLTextPage page) {
		ArrayList<TTSReadAction.TTSLineInfo> result = new ArrayList<TTSReadAction.TTSLineInfo>();
		if (page != null && !page.isEmptyPage()) {
			ArrayList<ZLTextLineInfo> lines = page.LineInfos;
			if (lines != null) {
				for (ZLTextLineInfo line : lines) {
					ZLTextParagraphCursor paragraph = line.ParagraphCursor;
					int startElement = line.StartElementIndex;
					...
					for (int i = startElement; i < endElement; i++) {
						...
						if (element instanceof ZLTextWord) {
							ZLTextWord word = (ZLTextWord) element;
							lineSb.append(new String(word.Data, word.Offset, word.Length));
						}
					}
					if (lineSb.length() > 0) {
						result.add(new TTSReadAction.TTSLineInfo(
								lineSb.toString(),
	...
	private String getParagraph() {
		...
		int start = myCurrentPage.StartCursor.getCharIndex();
		ZLTextParagraphCursor nextParagraph = myCurrentPage.StartCursor.getParagraphCursor();
		while (start < nextParagraph.getParagraphLength()) {
			ZLTextElement element = nextParagraph.getElement(start);
			if (element instanceof ZLTextWord) {
				ZLTextWord word = (ZLTextWord) element;
				paragraph.append(new String(word.Data, word.Offset, word.Length));
			}
			start++;
		}
	...
	//region bookmark
	public ZLTextElementArea getStartPosition(Bookmark bookmark) {
		if (bookmark == null || myCurrentPage == null || myCurrentPage.TextElementMap.areas() == null) {
			return null;
		}
		List<ZLTextElementArea> areas = myCurrentPage.TextElementMap.areas();
		for (ZLTextElementArea area : areas) {
			if (area != null && area.ParagraphIndex == bookmark.ParagraphIndex && bookmark.ElementIndex == area.ElementIndex && bookmark.CharIndex == area.CharIndex) {
				return area;
			}
		}
		return null;
	...
	public ZLTextElementArea getEndPosition(Bookmark bookmark) {
		if (bookmark == null || bookmark.getEnd() == null || myCurrentPage == null || myCurrentPage.TextElementMap.areas() == null) {
			return null;
		}
		List<ZLTextElementArea> areas = myCurrentPage.TextElementMap.areas();
		ZLTextPosition endPosition = bookmark.getEnd();
		for (ZLTextElementArea area : areas) {
			if (area != null && area.ParagraphIndex == endPosition.getParagraphIndex() && area.ElementIndex == endPosition.getElementIndex()) {
				return area;
			}
		}
		return null;
